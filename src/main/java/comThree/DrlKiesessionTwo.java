...
public class DrlKiesessionTwo {
    private static int thread_num = 2000;//线程数,设置同时并发线程数
    ...
    public static void main(String[] args) {
        ...
        helper2.addContent(myRuleFile1, ResourceType.DRL);
        ...
        for (int index = 0; index < client_num; index++) {
            Runnable run = () -> {
                try {
                    ...
                    kieSession1.dispose();//这里可注释 可不注释，因为每次Kiesession1都是不同的
                    kieSession2.dispose();//这里可注释 可不注释，因为每次Kiesession2都是不同的
